...
@Component
@Transactional
public class OrderRecipeRepositoryImpl implements OrderRecipeRepository {

	@Autowired
	EntityManager em;

	public List<OrderRecipe> findByMenuCode(int menuCode) {
		String sql = "SELECT r.recipe_code,r.recipe_order,r.recipe_detail "
				+ "FROM recipe r JOIN menu m ON r.menu_code = m.menu_code "
				+ "WHERE m.menu_code = :menu_code";
		Query query = em.createNativeQuery(sql, OrderRecipe.class);
		query.setParameter("menu_code", menuCode);
		@SuppressWarnings("unchecked")
		List<OrderRecipe> orderRecipe = query.getResultList();
		return orderRecipe;
	}
